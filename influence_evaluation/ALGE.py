...
def ALGE_C(G,data_memory):
    ...
    return ken_ALGE_C,rank_C,pre_sort2,node_rank_C,pre_I_with_node

def calculate_ken(G,pre_gat,data_memory,n=0):
...
def train(train_nodes, model, data_memory, g, node_features,G):
    ...
    for i in range(1,len(value_)):
        if value_[i]<value_[i-1]:
            ...
            rank.append(rank[-1])
    ken_pre2,pre_sort,node_rank_pre = calculate_ken(G, pre_gat, data_memory)
